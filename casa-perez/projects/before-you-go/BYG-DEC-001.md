...
**Naming-timeline correction (2026-08-01, per Solara):** an earlier version of this note speculated that the Context/Preparation content here might connect to an *earlier* "Find the Words" phase that predated the Foundation v1.1 reconciliation. That direction was backwards. Per Solara: **Before You Go was the original working title; Find the Words is the later/current internal project name.** This does not resolve the lineage question — it's still unknown whether this Context/Preparation material is a later evolution built on top of Foundation v1.1, a parallel/experimental branch, or was superseded before implementation — but any future investigation should start from the corrected timeline direction, not the reversed one.
